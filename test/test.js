...
describe("ErcOrdinal", function () {
  ...
  it("Buy: User type ETH first when buying", async function () {
    ...
    const buy = await router.swapExactETHForTokens(
      1,
      ["0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2", ercordinal.address],
      addr1.address,
      deadline2,
      {
        value: hre.ethers.utils.parseEther("0.0203"),
      }
    );
    const receipt = await buy.wait(1);
    const endingBalance = await ercordinal.getAddressToIds(addr1.address);
